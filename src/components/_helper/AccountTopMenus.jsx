...
export default function AccountTopMenus({
  ...
  return (
    <div className="account-top-menus mb-2">
      {Boolean(closing_req) && (
        <span className="account-top-menus__message text-danger">
          {t('common.account_closing_msg')}
        </span>
      )}
      {!closing_req && prefix && Boolean(is_approved) && !is_acc_closed && (
        <>
          {((prefix === 'saving' && checkPermission('make_saving_transactions', authPermissions)) ||
            ...
            <div className="account-top-menus__item">
              <PrimaryBtn
