...
export default function RolePermissions() {
  ...
  const {
    data: {
      data: { allPermissions, rolePermissions } = {
        allPermissions: [],
        rolePermissions: []
      }
    } = [],
  ...
  const setChange = (groupName, permission, isChecked) => {
  ...
  const setGroupChecked = (groupPermissions) => {
    return Object.keys(groupPermissions).every((permission) => groupPermissions[permission])
  }
  ...
  const updatePermissions = (event) => {
    ...
    Object.keys(permissions).forEach((group) => {
      Object.keys(permissions[group]).forEach((permission) => {
        if (permissions[group][permission]) selectedPermissions.push(permission)
      })
    })
