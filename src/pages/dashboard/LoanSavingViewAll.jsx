...
export default function LoanSavingViewAll() {
  ...
  const { data: { data: collections = [] } = {}, isLoading } = useFetch({
    action: 'dashboard/loan-saving'
  })

  const breadcrumbs = [
    { name: t('menu.dashboard'), path: '/', icon: <Home size={16} />, active: false },
    { name: t('dashboard.cards.Loan_Saving_Collections'), icon: <MyLoan size={16} />, active: true }
  ]
  ...
  return (
    <section className="staff">
      ...
      {isLoading ? (
        ...
        <ReactTable
          title={t('dashboard.cards.Loan_Saving_Collections')}
          columns={columns}
          data={collections}
        />
      )}
