    ...
    <section className="staff internal-audit-report">
      ...
      <div className="card mb-3 audit-filter-card">
        <div className="card-body">
          ...
          {filterError && <p className="text-danger mt-2 mb-0">{filterError}</p>}
      ...
      {isLoading ? (
        <div className="card audit-state-card">
          <div className="card-body">
        ...
        <div className="audit-content">
          <div className="audit-overview mb-3">
            <div className="overview-head">
              <h2 className="heading mb-1">{t('audit_report_page.internal.ui.overview_title')}</h2>
              <p className="subheading mb-0">{t('audit_report_page.internal.ui.overview_hint')}</p>
            </div>
            <div className="audit-metrics-grid">
              <div className="audit-metric-card metric-savings">
                ...
                <p className="metric-id">
                  {t('audit_report_page.internal.cards.card_number', {
                    value: tsNumbers('1201')
                  })}
                </p>
          ...
          <div className="audit-summary-grid mb-3">
            <div className="card report-table audit-section-card audit-summary-table">
                    ...
                    {loansByCategory.map((item) => (
                      <tr key={item.categoryId}>
                        ...
                        <td className="text-end">
                          {formatPercent(
                            toRecoveryPercentage(item.totalLoanRecovery, item.totalLoanGivenActual)
                          )}
                        </td>
                        ...
                        <td className="text-center">{renderMatchBadge(item.interestMismatch)}</td>
                      </tr>
                    ))}
                    <tr className="table-total-row">
                      <td>{t('common.total')}</td>
                      <td className="text-end">{money(loanSummaryTotals.totalLoanGivenActual)}</td>
                      ...
                      <td className="text-end">{money(loanSummaryTotals.totalLoanRemaining)}</td>
                      <td className="text-end">
                        {money(loanSummaryTotals.totalLoanGivenCalculated)}
                      </td>
                      <td className="text-center">-</td>
                      <td className="text-end">{money(loanSummaryTotals.totalInterestActual)}</td>
                      <td className="text-end">{money(loanSummaryTotals.totalInterestRecovery)}</td>
                      <td className="text-end">
                        {formatPercent(
                          toRecoveryPercentage(
          ...
          <div className="audit-profit-grid mb-3">
            <div className="profit-hero-card">
              <p className="hero-label mb-1">{t('audit_report_page.internal.profit_loss.title')}</p>
              ...
              <p className="hero-formula mb-2">
                {t('audit_report_page.internal.profit_loss.formula')}
              </p>
              <div className="profit-audit-values">
                <p className="mb-1">
                  <span>{t('audit_report_page.internal.profit_loss.api_total')}:</span>{' '}
                  <strong>{money(safeTotals.profitLoss)}</strong>
                </p>
                <p className="mb-2">
                  <span>{t('audit_report_page.internal.profit_loss.calculated_total')}:</span>{' '}
                  <strong>{money(calculatedTotals.profitLoss)}</strong>
                </p>
                {renderMatchBadge(isMismatch(safeTotals.profitLoss, calculatedTotals.profitLoss))}
              </div>
            </div>

            <div className="card report-table audit-section-card profit-breakdown-card mb-0">
              <div className="card-header">
                <h2 className="heading mb-1">
                  ...
                  <table className="table table-hover table-report table-breakdown mb-0">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className="text-end">{money(safeTotals.totalLoanSavingsAll)}</td>
                        <td className="text-end">{money(calculatedTotals.totalLoanSavingsAll)}</td>
                        <td className="text-center">
                          {renderMatchBadge(
                            isMismatch(
